...
import { ExternalLink, Globe, Smartphone, Code, X, ChevronRight, CheckCircle2 } from 'lucide-react';
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-20 bg-zinc-900/50 border-y border-zinc-850 relative">
      <div className="absolute top-1/3 left-10 w-[300px] h-[300px] bg-indigo-500/5 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 text-xs font-mono uppercase tracking-wider">
            <Code className="w-3 h-3 text-indigo-400" /> Showcase
          </div>
          ...
          {(['all', 'web', 'mobile'] as const).map((type) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200 uppercase tracking-wider cursor-pointer border ${
                filter === type
                  ? 'bg-gradient-to-r from-indigo-500 to-violet-600 text-white border-transparent shadow-lg shadow-indigo-500/10'
          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project, idx) => (
              <motion.div
                ...
                key={project.id}
              >
                <TiltCard glowColor="rgba(139, 92, 246, 0.16)" className="h-full">
                  <div
                    className="group relative bg-zinc-950/80 border border-zinc-850 hover:border-zinc-750 p-6 rounded-2xl flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/5 h-full"
                  >
                    <div>
                      {/* Category & Date */}
                    ...
                    <div className="space-y-4 pt-4 border-t border-zinc-900">
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {project.technologies.length > 3 && (
                          <span className="text-[10px] font-mono text-indigo-400 bg-indigo-950/20 border border-indigo-900/20 px-2 py-0.5 rounded">
        ...
        {/* Spring-Loaded Modal Detail View */}
        <AnimatePresence>
          {selectedProject && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col z-10"
              >
                ...
                <div className="p-6 border-b border-zinc-800 flex items-start justify-between bg-zinc-950">
                  <div>
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="text-xs font-mono text-indigo-400 font-semibold uppercase bg-indigo-950/45 px-2 py-0.5 rounded border border-indigo-900/30">
                ...
                <div className="p-6 border-t border-zinc-850 bg-zinc-950 flex items-center justify-between">
                  <button
                    onClick={() => setSelectedProject(null)}
                    className="px-4 py-2 border border-zinc-800 hover:border-zinc-750 hover:bg-zinc-900 text-zinc-300 font-semibold rounded-xl text-sm transition-all cursor-pointer"
                  >
                    Close Details
                  </button>
                  ...
                  {selectedProject.link && selectedProject.link !== '#' && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-2 bg-gradient-to-r from-indigo-500 to-violet-600 hover:from-indigo-400 hover:to-violet-500 text-white font-semibold px-5 py-2 rounded-xl text-sm shadow-md cursor-pointer"
                    >
                      <ExternalLink className="w-4 h-4" />
