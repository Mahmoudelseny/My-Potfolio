import { ArrowRight, Mail, Github, FileText, Globe, Code, Layers, Sparkles, Linkedin } from 'lucide-react';
...
export default function Hero({ onNavigate, onOpenResumeModal }: HeroProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-10 left-10 w-[300px] h-[300px] bg-teal-500/5 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-mono tracking-wider uppercase animate-fade-in">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {mainTechs.map((tech) => (
                <span
                  key={tech}
                  className="px-3 py-1 rounded-md bg-zinc-900 border border-zinc-800 text-zinc-300 text-xs font-mono"
                >
                  {tech}
                </span>
              ))}
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button
                ...
                className="group flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold px-6 py-3.5 rounded-xl shadow-lg transition-all duration-200 cursor-pointer"
              >
                View Selected Work
          ...
          <div className="lg:col-span-5 relative mt-6 lg:mt-0">
            <div className="absolute inset-0 bg-gradient-to-tr from-emerald-500/10 to-teal-500/10 rounded-2xl blur-xl opacity-70"></div>
            
            <div className="relative bg-zinc-900/90 border border-zinc-800 p-6 rounded-2xl shadow-2xl space-y-6">
              
              {/* Header profile info */}
              <div className="flex items-center gap-4 border-b border-zinc-800 pb-5">
                <div className="w-16 h-16 rounded-xl bg-gradient-to-tr from-emerald-500 to-teal-600 flex items-center justify-center text-zinc-950 text-2xl font-black shadow-lg">
                  MS
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-zinc-950/60 border border-zinc-850 rounded-xl">
                  <span className="block text-3xl font-black text-emerald-400">2+</span>
                  <span className="text-xs text-zinc-400 font-mono mt-1 block">Native Frameworks</span>
                </div>
              </div>

              {/* Education Snippet */}
              <div className="p-4 bg-emerald-950/20 border border-emerald-900/30 rounded-xl flex gap-3 items-start text-left">
                <span className="text-2xl mt-0.5">{PERSONAL_INFO.education.logo}</span>
                <div>
                  <h4 className="font-bold text-sm text-emerald-400">{PERSONAL_INFO.education.institution}</h4>
