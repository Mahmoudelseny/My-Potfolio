...
import { Menu, X, FileText, Download, Briefcase, Code, Award, MessageSquare, BookOpen } from 'lucide-react';
...
export default function Header({ activeSection, onNavigate, onOpenResumeModal }: HeaderProps) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-1 lg:space-x-2">
            ...
            <button
              ...
              className="ml-4 flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-zinc-950 font-semibold px-4 py-2 rounded-lg text-sm shadow-md transition-all duration-200 cursor-pointer hover:shadow-emerald-500/10 hover:scale-[1.02]"
            >
          ...
          <div className="flex md:hidden items-center gap-3">
            <button
              onClick={onOpenResumeModal}
              className="flex items-center gap-1 bg-emerald-500 text-zinc-950 font-semibold px-3 py-1.5 rounded-lg text-xs transition-all duration-200"
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-zinc-950/95 backdrop-blur-lg border-b border-zinc-850 px-4 pt-2 pb-6 space-y-2">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeSection === item.id;
            return (
