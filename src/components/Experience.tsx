...
export default function Experience() {
  return (
    <section id="experience" className="py-20 bg-zinc-950 relative">
      <div className="absolute top-1/2 right-10 w-[300px] h-[300px] bg-teal-500/5 rounded-full blur-3xl pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-mono uppercase tracking-wider">
            <Briefcase className="w-3 h-3" /> Timeline
          </div>
        ...
        {/* Timeline Structure */}
        <div className="max-w-4xl mx-auto relative border-l border-zinc-800 ml-4 md:ml-32 pl-6 md:pl-12 space-y-12">
          ...
          {EXPERIENCE_HISTORY.map((exp, index) => {
            return (
              <div key={exp.id} className="relative group text-left">
                ...
                {/* Timeline dot */}
                <div className="absolute -left-[31px] md:-left-[55px] top-1.5 w-4 h-4 rounded-full border-2 border-emerald-500 bg-zinc-950 group-hover:bg-emerald-400 transition-colors duration-200 shadow-md shadow-emerald-500/20"></div>
                ...
                <div className="bg-zinc-900/40 border border-zinc-850 hover:border-zinc-800 p-6 rounded-2xl transition-all duration-300 hover:bg-zinc-900/60 shadow-lg hover:shadow-xl">
                  ...
                  <div className="inline-block md:hidden mb-3">
                    <span className="text-xs font-mono font-bold text-emerald-400 bg-emerald-950/25 border border-emerald-900/30 px-2.5 py-1 rounded-lg">
                      {exp.date}
                    </span>
                  </div>
                  ...
                  <ul className="space-y-2.5 text-zinc-300 text-sm leading-relaxed">
                    {exp.bulletPoints.map((bp, bpIdx) => (
                      <li key={bpIdx} className="flex gap-2 items-start">
                        <span className="text-emerald-500 shrink-0 select-none mt-1">
